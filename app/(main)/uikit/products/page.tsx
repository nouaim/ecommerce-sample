...
import { getProducts, getCategories } from "@/demo/service/api";
...
const ListDemo = () => {
    ...
    const getFilteredProducts = () => {
        if (selectedCategory) {
            return dataViewValue.filter(product => product.category === selectedCategory);
        }
                ...
                {loading ? (
                    ...
                    categories?.map((category: string) => (
                        <Button
                            ...
                            className={`whitespace-nowrap ${selectedCategory === category ? '' : 'p-button-outlined'}`}
                        />
                    ))
                )}
            </div>
            ...
            >
                <div className="p-4 border-1 surface-border border-round hover:shadow-2 transition-all transition-duration-300 h-full flex flex-column">
                    ...
                    <div className="flex flex-column align-items-center text-center mb-3 flex-grow-1">
                        ...
                        />
                        <div className="text-xl font-bold mb-2 line-clamp-2">{product.title}</div>
                        <Rating 
                            value={product.rating.rate} 
                            readOnly 
                            cancel={false} 
    ...
    const itemTemplate = (product: Product) => {
        ...
        return layout === "list" ? dataviewListItem(product) : dataviewGridItem(product);
    };
